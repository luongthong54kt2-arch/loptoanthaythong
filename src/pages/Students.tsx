...
export default function Students() {
  const { students, classes, enrollments, loadStudents, loadClasses, loadEnrollments,
    addStudent, updateStudent } = useDataStore()
    ...
    <div className="space-y-6">
                ...
                <tr key={s.id} className={`border-b border-teal-50 hover:bg-teal-50/50 transition-colors ${i % 2 === 0 ? 'bg-white' : 'bg-teal-50/20'}`}>
                  ...
                  <td className="px-4 py-3">
                    {(s as any).password ? (
                      ...
                      <span className="inline-flex items-center gap-1 text-xs font-bold text-red-500 bg-red-50 border border-red-200 px-2 py-0.5 rounded-full">
                        ✗ Chưa có
                      </span>
                    )}
                  </td>
      ...
      <Modal open={modal === 'form'} onClose={() => setModal(null)}
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
          ...
          {/* ── Hàng 1: Mã HS / Họ tên / Trạng thái ── */}
          <div>
          ...
          {/* ── Hàng 2: Mật khẩu (bắt buộc, full-width) ── */}
