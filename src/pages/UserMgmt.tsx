...
import { supabase } from '@/lib/supabase' // ✅ Gọi trực tiếp Supabase để chống treo
...
export default function UserMgmt() {
  const { profiles, loadProfiles } = useDataStore()
  ...
  // ✅ SỬA LỖI TREO: Gọi trực tiếp Supabase để Cập nhật vai trò
  const setRole = async (id: string, role: Role) => {
    ...
    try {
      ...
      loadProfiles() // Tải lại ngầm, KHÔNG dùng await để chống kẹt
    } catch (e: any) { 
      toast.error(e.message || 'Lỗi cập nhật vai trò, kiểm tra lại quyền Admin!') 
    }
    finally { 
      setSaving(null) // Chắc chắn tắt xoay
    }
  }

  // ✅ SỬA LỖI TREO: Gọi trực tiếp Supabase để Khóa/Bật
  const toggleActive = async (id: string, active: boolean) => {
    ...
    try {
      ...
      toast.success(!active ? 'Đã kích hoạt lại tài khoản!' : 'Đã khóa tài khoản thành công!')
      loadProfiles() // Tải lại ngầm, KHÔNG dùng await
    } catch (e: any) { 
      toast.error(e.message || 'Lỗi khóa tài khoản, kiểm tra lại quyền Admin!') 
    }
    finally { 
      setSaving(null) // Chắc chắn tắt xoay
    }
  ...
  // ✅ Hàm xử lý XÓA người dùng (Gọi trực tiếp Supabase)
  const handleDeleteUser = async (id: string, name: string, email: string) => {
    const confirmMsg = `Thầy có chắc chắn muốn XÓA VĨNH VIỄN người dùng:\n${name || email}?\n\nHành động này không thể hoàn tác!`
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {profiles.length === 0 && (
                <tr><td colSpan={6} className="text-center py-12 text-gray-400 font-medium">Chưa có người dùng nào trong hệ thống</td></tr>
              )}
              {profiles.map((p, i) => (
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Thêm người dùng mới" size="md">
        <div className="space-y-4">
          ...
          <div className="flex gap-3 mt-6 justify-end border-t border-gray-100 pt-5">
            ...
            <button onClick={() => { void handleCreateUser() }} disabled={creating} className="btn-teal px-8 py-2.5 rounded-xl font-bold flex items-center gap-2 shadow-lg">
